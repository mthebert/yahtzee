...
let rollSound = new Audio('media/die_roll.wav')
function rollDice() {
  rollCount++
...
function changeDiceFaces(randomDice) {
  for (let i = 0; i < diceElements.length; i++) {
    ...
    if (diceElements[i].classList.contains('active')) {
      ...
      face.src = 'media/dice' + randomDice[i] + '.png'
    }
  }
}
function resetDiceFaces() {
  for (let i = 0; i < diceElements.length; i++) {
    ...
    let diceNumber = i + 1
    face.src = 'media/dice' + diceNumber + '.png'
  }
}
diceElements.forEach(function (diceElement, index) {
...
function onCellClick() {
  ...
  if (
    rollCount == 0 ||
    row === null ||
    (onlyPossibleRow != 'blank' && row != onlyPossibleRow)
  )
    return
...
function calculateLowerSectionScore(playerScore) {
  ...
  if (yahtzee > 0) {
    ...
    yahtzee = parseInt(
      document.getElementById('yahtzee' + playerNumber).innerHTML,
    )
  }
  lowerSectionScore =
    threeOfAKind +
